...
pilha_fixa_linear_t *criar_pilha_fixa_linear_ret(unsigned long capacidade) {
    ...
    pilha_ptr = malloc(sizeof(pilha_fixa_linear_t));
    if (!pilha_ptr) {
        return NULL; /* retorna NULO se a alocação falhou */
    }

    /* usamos o macro MACRO_PILHA_FIXA_LINEAR_TIPO para alocar o espaço necessário
       para a quantidade de dados necessários, e fazemos type-casting
       para o tipo esperado. */
    pilha_ptr->dados_ptr = (MACRO_PILHA_FIXA_LINEAR_TIPO*)calloc(capacidade, 
        sizeof(MACRO_PILHA_FIXA_LINEAR_TIPO));
...
MACRO_PILHA_FIXA_LINEAR_TIPO retirar_pilha_fixa_linear_ret(pilha_fixa_linear_t* pilha_ptr) {
    MACRO_PILHA_FIXA_LINEAR_TIPO valor;
...
void reiniciar_pilha_fixa_linear(pilha_fixa_linear_t* pilha_ptr) {
    ...
    memset(pilha_ptr->dados_ptr, 0, sizeof(MACRO_PILHA_FIXA_LINEAR_TIPO) * pilha_ptr->capacidade);
}
