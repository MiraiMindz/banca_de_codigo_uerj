...
fila_linear_fixa_t *criar_fila_fixa_ret(unsigned long capacidade) {
    ...
    fila = malloc(sizeof(fila_linear_fixa_t));
    if (!fila) {
        return NULL; /* retorna NULO se a alocação falhou */
    }

    /* usamos o macro MACRO_FILA_FIXA_TIPO para alocar o espaço necessário
       para a quantidade de dados necessários, e fazemos type-casting
       para o tipo esperado. */
    fila->dados_ptr = (MACRO_FILA_FIXA_TIPO*)calloc(capacidade, 
        sizeof(MACRO_FILA_FIXA_TIPO));
    if (!fila->dados_ptr) {
